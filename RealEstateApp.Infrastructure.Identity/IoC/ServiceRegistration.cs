...
namespace RealEstateApp.Infrastructure.Identity.IoC
{
    public static class ServiceRegistration
    {
        ...
        public static void AddIdentityInfrastructureForWeb(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            {
                options.AddPolicy("ExcludeAdmin&Agent", policy =>
                    policy.RequireAssertion(context =>
                        context.User.IsInRole("Client") || !context.User.Identity.IsAuthenticated));
            });
            ...
            services.AddAuthorization(options =>
            {
                options.AddPolicy("ExcludeAdmin", policy =>
                    policy.RequireAssertion(context =>
                        context.User.IsInRole("Client") || context.User.IsInRole("Agent") || !context.User.Identity.IsAuthenticated));
            });
            #endregion
